...
def predictive_multiclass_data(meta_dataframes): #turning probabilistic multiclass meta data into predictions
    predictive_meta_dataframes = []
    for i,df in enumerate(meta_dataframes):
        if df.columns.names[-1] == "class": #multiclass check
            pred_frame = df.loc[:, df.columns.get_level_values(0) != "labels"].groupby(level=[i for i in range(df.columns.nlevels)], axis=1).idxmax()
            ...
            predictive_meta_dataframes.append(pred_frame)
    
    return  predictive_meta_dataframes
...
def min_class_prev(y):
    '''
    For stratifying time series data with respect to the prevalence across time of the least prevalent class.
     Assumes ordinally labeled classes
    '''
    classes, prevalences = np.unique(y, return_counts=True)
    min_class = np.argmin(prevalences)
    min_class_prevs = np.array([np.sum(y_seq==min_class) for y_seq in y])

    return min_class_prevs


def sample(X, y, strategy, random_state, time_series=False):
    if strategy is None:
    ...
    if (strategy == "undersampling") or (strategy == "oversampling"):
        if time_series==True:
            X_resampled, _ = sampler.fit_resample(X=X, y=min_class_prev(y))
