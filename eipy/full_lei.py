# Version of LEI that allows for variable length sequences and extra modality data
...
from eipy.multiclass_metrics import (
    mc_base_summary,
    mc_ensemble_summary,
)
...
class EnsembleIntegration:
    ...
    groups : list, default=None
        Iterable for keeping track of samples and time points during nested CV
    ...
    def __init__(
        ...
        self.cv_outer = StratifiedGroupKFold(
    ...
    def fit_base(self, X, y, base_predictors=None, modality_name=None, X_mode=None, y_mode=None):
        ...
        y = y_to_numpy(y)
        self.num_classes = len(np.unique(y)) #refactor to use this attribute
    ...
    def fit_ensemble(self, ensemble_predictors=None):
        ...
        num_classes = len(np.unique(y_test_combined))
        ...
        for model_name, model in tqdm(
            ...
            for fold_id in range(self.k_outer):
                ...
                if self.sampling_aggregation == "mean":
                    ...
                    else: # maintain class specific columns for multiclass
                        X_train = X_train.T.groupby(level=[0, 1, -1]).mean().T
                        X_test = X_test.T.groupby(level=[0, 1, -1]).mean().T                        
            ...
            if num_classes ==2: #binary classification
                ...
                for class_id in range(num_classes):
                    ensemble_predictions[model_name, class_id] = [pred[class_id] for pred in y_pred_combined]
        ...
        if num_classes !=2: #multiclass
            self.ensemble_predictions = self.ensemble_predictions.rename(columns={'labels': ('labels','')})
            self.ensemble_predictions.columns = pd.MultiIndex.from_tuples(self.ensemble_predictions.columns, names=['ensemble predictor', 'class'])

            self.ensemble_summary = mc_ensemble_summary(self.ensemble_predictions)
        else:
        ...
        if self.model_building:
            for model_name, model in tqdm(
                ...
                if self.sampling_aggregation == "mean":
                    if len(np.unique(y_train)) != 2: # maintain class specific columns for multiclass
                        X_train = X_train.T.groupby(level=[0, 1, -1]).mean().T
                        X_test = X_test.T.groupby(level=[0, 1, -1]).mean().T                        
                    else:
                        X_train = X_train.T.groupby(level=[0, 1]).mean().T
    ...
    def predict(self, X_dict, ensemble_model_key):
        ...
        if self.sampling_aggregation == "mean":
            if len(X.columns[0]) == 4: #4 level multi-index, aka multiclass
                ensemble_prediction_data = (
                    ensemble_prediction_data.T.groupby(level=[0, 1, -1]).mean().T
            )
            else:
                ensemble_prediction_data = (
    ...
    def _fit_base(self, X, y, base_predictors=None, modality_name=None, X_mode=None, y_mode=None):
        ...
        #multiclass
            ...
            self.base_summary = mc_base_summary(self.ensemble_test_data)
    ...
    def _fit_base_inner(
        ...
        with Parallel(
            ...
            for _outer_fold_id, [(train_index_outer, _test_index_outer),(train_index_outer_mode, _test_index_outer_mode)] in enumerate(
                tqdm(
                    zip(cv_outer.split(X, y, groups=self.groups), cv_outer_split_mode),
                ...
                groups_inner = self.groups[train_index_outer]
                ...
                output = parallel(
                    ...
                    for inner_fold_params in enumerate(
                        zip(cv_inner.split(X_train_outer, y_train_outer, groups=groups_inner), cv_inner_split_mode)
                    )
        ...
        with Parallel(
            ...
            output = parallel(
                ...
                for outer_fold_params in enumerate(
                    tqdm(zip(cv_outer.split(X, y, groups=self.groups), cv_outer_split_mode),
