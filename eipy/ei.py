...
class EnsembleIntegration:
    ...
    def __init__(
        ...
        self.time_series = time_series[0]
        self.time_point = time_series[1]
        ...
        if (self.time_series) and (self.sampling_strategy==None): #to ensure consistent sampling across time points
            self.cv_outer = KFold(
    ...
    def _fit_base(self, X, y, base_predictors=None, modality_name=None):
        ...
        # binary
        if self.time_series:
            n_unique_labels = len(set(y[:,self.time_point]))
        else:
    ...
    def _fit_base_inner(
        ...
        if (self.time_series) and (self.sampling_strategy!=None):
            y_for_splitting_outer = min_class_prev(y)
        else:
            y_for_splitting_outer = y
        ...
        with Parallel(
            ...
            for _outer_fold_id, (train_index_outer, _test_index_outer) in enumerate(
                tqdm(
                    cv_outer.split(X, y_for_splitting_outer, groups=np.concatenate(4*[np.arange(len(X)//4)])),
                ...
                if (self.time_series) and (self.sampling_strategy!=None):
                    y_for_splitting_inner = min_class_prev(y_train_outer)
                else:
                    y_for_splitting_inner = y_train_outer
                ...
                output = parallel(
                    ...
                    for inner_fold_params in enumerate(
                        cv_inner.split(X_train_outer, y_for_splitting_inner)
                    )
    ...
    def _fit_base_outer(
        ...
        with Parallel(
            ...
            output = parallel(
                ...
                for outer_fold_params in enumerate(cv_outer.split(X, y_for_splitting_outer, np.concatenate(4*[np.arange(len(X)//4)])))
                for sample_state in enumerate(self.random_numbers_for_samples)
            )
    ...
    def _train_predict_single_base_predictor(
        ...
        if self.time_series:
            y_sample = y_sample[:, self.time_point]
            y_test = y_test[:, self.time_point]
