...
from eipy.mc_perm_importance import MultiClassPermutationImportance as mcpi
    ...
    def local_model_rank(self, ensemble_predictor_keys):
        ...
        for model_name, model in tqdm(
            ...
            if ("Mean" in model_name) or ("Median" in model_name):
                ...
                if len(np.unique(ensemble_y_train)) == 2: #binary
                    ...
                    base_predictor_list = [column_name[1] for column_name in ensemble_X_train.columns]
                    modality_list = [column_name[0] for column_name in ensemble_X_train.columns]
                    ...
                    pi = mcpi(
                        ...
                        n_repeats=self.n_repeats,
                        n_jobs=-1)
                    pi.mc_permutation_importance()
                    
                    base_predictor_modality_list = list(set([column_name[:-1] for column_name in ensemble_X_train.columns]))
                    base_predictor_list = [x[1] for x in base_predictor_modality_list]
                    modality_list = [x[0] for x in base_predictor_modality_list]
            ...
            pi_df = pd.DataFrame(
                {
                    ...
                    "base predictor": base_predictor_list,
